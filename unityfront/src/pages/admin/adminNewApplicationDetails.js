...
let profileImageGet=`${config.SpingUrl}/customer/getProfileImage/${customer.id}`
  ...
  const accept=(customerPolicyId)=>{
    axios.put(`${config.SpingUrl}/admin/acceptPolicy/${customerPolicyId}`).then(
     (response)=>{
       toast.success("Policy Accepted SuccessFully")
       Navigate("/newapplications", {state:{admin:admin}})
     }
    ).catch((error)=>{
   ...
   const reject=(customerPolicyId)=>{
    axios.delete(`${config.SpingUrl}/admin/rejectPolicy/${customerPolicyId}`).then(
     (response)=>{
  ...
  <tbody style={styles.myfont}>
    ...
    <tr>
                ...
                <table className='table' style={{ width: '60%' }}>
                    <tr>
                        <td>
                            <button className='btn btn-success mt-4' type="submit" onClick={()=>{accept(customerPolicy.id)}} style={{ width: '350px', borderRadius: '15px',backgroundColor:'green' }}>Accept</button>
                        </td>
                        <td>
                            <button className='btn btn-danger mt-4' type="submit"onClick={()=>{reject(customerPolicy.id)}} style={{ width: '350px', borderRadius: '15px',backgroundColor:'red' }}>Reject</button>
                            </td>
                        </tr>
                </table>
                </center>
