...
const Newapplications=()=>{
  ...
  if(!token)
  {
    ...
    axios.get(`${config.SpingUrl}/admin/getNewAppilcations`).then((response)=>{
    ...
    return(
      ...
      <div style={{flex:"1 1 auto", display:"flex", flexFlow:"column", height:"100vh", overflowY:"auto"}}>
        ...
        <h1 style={{marginBottom:'40px',marginLeft:'10px',textAlign:'center'}}><b>New Applications</b></h1>
      ...
      <tbody>
             {customerPolicies.map((customerpolicy)=>{
              ...
              <td>{customerpolicy.customer.firstName} {customerpolicy.customer.lastName}</td>
                ...
                <td><Link to='/adminNewApplicationDetails' className='btn btn-primary' state={{admin:admin,customerPolicy:customerpolicy}} style={styles.button}>View Details</Link></td>
              </tr>
              )
             })
