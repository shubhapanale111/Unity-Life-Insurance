...
const AdminSurrenderApplicationDetails =()=>{
...
let profileImageGet=`${config.SpingUrl}/customer/getProfileImage/${customer.id}`
  ...
  const calculateSurrenderAmount=()=>{
    let noOfPremiums=calculateMonth(new Date(customerPolicy.policyStartDate),new Date(customerPolicy.premiumDate));
    let premium=customerPolicy.premium;
    return (0.40*premium*noOfPremiums);
  }
  ...
  const accept=(customerPolicyId)=>{
    surrenderAmount=calculateSurrenderAmount();
    console.log(surrenderAmount)
    axios.put(`${config.SpingUrl}/admin/acceptSurrender/${customerPolicyId}`,{},{
        params:{surrenderAmount}
    }).then(
     (response)=>{

       toast.success("Policy Surrender Granted")
       Navigate("/adminSurrenderApplications", {state:{admin:admin}})
     }
    ).catch((error)=>{
      toast.error("Policy Surrender Accept Failed"+error)
    })
   }
   const reject=(customerPolicyId)=>{
  
    axios.put(`${config.SpingUrl}/admin/rejectSurrender/${customerPolicyId}`,{},{
        params:{surrenderAmount}}).then(
     (response)=>{
       toast.success("Policy Surrender Rejected")
       Navigate("/adminSurrenderApplications", {state:{admin:admin}})
     }
    ).catch((error)=>{
      toast.error("Policy Surrender Reject Failed"+error)
    })
   }
  useEffect(()=>{
  ...
  <tbody style={styles.myfont}>
    ...
    <td>Surrender Amount</td>
      <td>{calculateSurrenderAmount()} INR</td>
    </tr>
    <tr>
                ...
                <table className='table' style={{ width: '60%' }}>
                    <tr>
                        <td>
                            <button className='btn btn-success mt-4' type="submit" onClick={()=>{accept(customerPolicy.id)}} style={{ width: '350px', borderRadius: '15px',backgroundColor:'green' }}>Accept Surrender</button>
                        </td>
                        <td>
                            <button className='btn btn-danger mt-4' type="submit"onClick={()=>{reject(customerPolicy.id)}} style={{ width: '350px', borderRadius: '15px',backgroundColor:'red' }}>Reject Surrender</button>
                            </td>
                        </tr>
                </table>
                </center>
...
export default AdminSurrenderApplicationDetails
const styles={
    myfont: {
        textDecoration: "none",
        fontWeight: "bold",
