...
const AdminClaimApplications=()=>{
  ...
  if(!token)
  {
    ...
    axios.get(`${config.SpingUrl}/admin/getNewClaims`).then((response)=>{
    ...
    return(
      ...
      <div style={{flex:"1 1 auto", display:"flex", flexFlow:"column", height:"100vh", overflowY:"auto"}}>
        ...
        <h1 style={{marginBottom:'40px',marginLeft:'10px',textAlign:'center'}}><b>Applications For Claim</b></h1>
      ...
      <tbody>
             {customerPolicies.map((customerpolicy)=>{
              ...
              <td>{customerpolicy.customer.firstName} {customerpolicy.customer.lastName}</td>
                ...
                <td><Link to='/adminClaimApplicationDetails' className='btn btn-primary' state={{admin:admin,customerPolicy:customerpolicy}} style={styles.button}>View Details</Link></td>
              </tr>
              )
             })
...
export default AdminClaimApplications
const styles={
  button:  {
      borderRadius:'15px',
    }
